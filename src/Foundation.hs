...
instance Yesod App where
    ...
    isAuthorized (ListingR _)  _ = return Authorized
...
instance YesodAuth App where
    ...
    getAuthId creds =
      runDB $
      do x <- insertBy $ User Nothing (credsIdent creds) Nothing Nothing False
         return $
           Just $
           case x of
...
isAuthenticated :: Handler AuthResult = do
    muid <- maybeAuthId
    session <- getSession
    Debug.Trace.trace (show session) $ 
      return $ case muid of
   ...
   = do
    ...
    liftIO $
      renderSendMail
        ...
        , mailFrom = Address (Just "brbn trade") "wm@brbn.trade"
